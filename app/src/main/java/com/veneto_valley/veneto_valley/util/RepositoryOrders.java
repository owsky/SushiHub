...
public class RepositoryOrders {
    ...
    public RepositoryOrders(Application application) {
        ...
        orderDao = AppDatabase.getInstance(application).orderDao();
    ...
    public void confirmOrder(Order order, String table) {
        Executors.newSingleThreadScheduledExecutor().execute(() -> {
            ...
            if (!preferences.contains("is_master")) {
                Order payload = new Order(order.tableCode, order.dish, Order.OrderStatus.insertOrder, order.user, order.receivedFromSlave);
    ...
    public void undoConfirmOrder(Order order, String table) {
        Executors.newSingleThreadScheduledExecutor().execute(() -> {
            ...
            if (!preferences.contains("is_master")) {
                Order payload = new Order(order.tableCode, order.dish, Order.OrderStatus.deleteOrder, order.user, order.receivedFromSlave);
                Nearby.getInstance(true, application, table, getPayloadCallback(table))
                        .send(payload.getBytes());
            }
        });
    ...
    public void markAsDelivered(Order order, String table) {
        Executors.newSingleThreadScheduledExecutor().execute(() -> {
            ...
            update(order);
            if (!preferences.contains("is_master")) {
                Order payload = new Order(order.tableCode, order.dish, Order.OrderStatus.deliverOrder, order.user, order.receivedFromSlave);
    ...
    public void undoMarkAsDelivered(Order order, String table) {
        Executors.newSingleThreadScheduledExecutor().execute(() -> {
            ...
            update(order);
            if (!preferences.contains("is_master")) {
                Order payload = new Order(order.tableCode, order.dish, Order.OrderStatus.undoDeliverOrder, order.user, order.receivedFromSlave);
                Nearby.getInstance(false, application, table, getPayloadCallback(table))
                        .send(payload.getBytes());
            }
        });
